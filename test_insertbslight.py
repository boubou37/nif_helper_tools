...
stream = open('sam_texturetest_export.nif','rb')
data = NifFormat.Data()
data.read(stream)
...
for root in data.roots:
    for block in root.tree():
        if type(block) == NifFormat.NiTriShape:
            ...
            _init_shader_flags_1(bsl)
            _init_shader_flags_2(bsl)
            if bsst is None:
                bsst = NifFormat.BSShaderTextureSet()
            bsst.num_textures = 9
